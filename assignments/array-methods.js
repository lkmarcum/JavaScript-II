...
let largeShirts = [];
largeShirts = runners.filter(entry => entry.shirt_size === "L");
...
// Problem 1
// Organizers want to reward runners whose donation >250 with a special gift. Use .filter() to make a new array with the info of these big spenders.
let bigSpenders = [];
bigSpenders = runners.filter(element => element.donation >= 250);
console.log(bigSpenders);

// Problem 2
// Event director has been reading too much Harry Potter and strangely identifies with the villain, so he wants all runners' first names changed to 'Voldemort'
let heWhoShallNotBeNamed = [];
heWhoShallNotBeNamed = runners.map(item => item);
heWhoShallNotBeNamed.forEach(function(element) {
  element.first_name = "Voldemort";
});
console.log(heWhoShallNotBeNamed);
